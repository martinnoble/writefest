...
angular.module('myApp').controller('authorController',
    ['$scope', '$location', '$routeParams', '$http', 'filterFilter', 'ScriptService', 'Upload', '$timeout',
        function ($scope, $location, $routeParams, $http, filterFilter, ScriptService, Upload, $timeout) {
            ...
            $http.get('/api/script')
                .success(function (data, status) {
                    if(status === 200 && data.result){
                        $scope.scriptdata = data;
                        $scope.scriptcount = 0;

                        var arrayLength = $scope.scriptdata.scripts.length;
                        for (var i = 0; i < arrayLength; i++) {
                            if ($scope.scriptdata.scripts[i].status != 5) {
                                $scope.scriptcount++;
                            }

                            if ([1,5].indexOf($scope.scriptdata.scripts[i].status) != -1) {
                                $scope.scriptdata.scripts[i].editable = true;
                            } else {
                                $scope.scriptdata.scripts[i].editable = false;
                            }
                        }
                        $scope.scriptmax = 3;

                        $scope.cansubmit = $scope.scriptcount < $scope.scriptmax;
                       

                        if ($location.path().indexOf("script") != -1) {
                            ...
                            if (scriptid) {
                                ...
                                $scope.selectedScript = filterFilter($scope.scriptdata.scripts, {id: scriptid})[0];

                                if (!$scope.selectedScript) {
                                    $scope.$emit("error", "Failed to read script data");
                                    $location.url("/author");
                                }

                                if ([1,5].indexOf($scope.selectedScript.status) != -1) {
                                    $scope.selectedScript.editable = true;
                                }

                            } else {
                                author = $scope.scriptdata.author.id;
                                $scope.selectedScript = {editable: true, status: 1, id: 0, author: author, season: $scope.scriptdata.seasonid};
            ...
            if (error) {
                scope.$emit('error', "Failed to get script data");
            }

            $scope.submitNew = function() {
                $location.url("/author/script/");
            }

            $scope.modify = function(script) {
                $location.url("/author/script/" + script.id);
            }
            ...
            $scope.save = function() {
                ...
                if (data.status == 5) {
                    if (data.file) {
                        ...
                        $scope.$emit("error", "Cannot re-submit a script without uploading a new file");
                        valid = false;
                    }
                }
                ...
                if (!data.file) {
                    ...
                    ScriptService.update(data, action)
                        .then(function () {
                            $scope.$emit('info', "Script saved");
                            $location.url("/author");
                        })
                        .catch(function () {
                            $scope.$emit('error', "Failed to save script");
                            $scope.disabled = false;
                            $scope.registerForm = {};
                        });
...
angular.module('myApp').controller('adminController',
    ['$scope', '$location', 'AuthService', 'AdminService', '$route', '$routeParams', 'filterFilter', '$http',
        function ($scope, $location, AuthService, AdminService, $route, $routeParams, filterFilter, $http) {

            
            $scope.editEmail = function(email) {
                $location.url("/admin/email/" + email.id);
            }
            ...
            $scope.saveemail = function(email) {
                ...
                $http.post('/api/admin/email', {email: email})
                    .success(function(data, status) {

                        if (data['result']) {
                            $scope.$emit("info", "email content saved");
                            $location.url("/author");
                        } else {
                            $scope.$emit("error", "Failed to save email content");
                        }
                    })
                    .error(function() {
                        $scope.$emit("error", "Failed to save email content");
            ...
            AdminService.readData()
                .then(function() {
                    ...
                    if ($location.path().indexOf("email") != -1) {
                        $scope.selectedEmail = filterFilter($scope.admindata.emails, {id: $routeParams['id']})[0];
